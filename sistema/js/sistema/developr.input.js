...
;(function($, window, document, undefined)
{
	...
	$.fn.styleSelect.defaults = {
		 ...
		 * @var string|boolean
		 */
		multipleValuesText: '%d seleccionados',
		 ...
		 * @var string|boolean
		 */
		allValuesText: 'Seleccionados todos',
	...
	doc.on('mousedown touchstart', 'span.switch', function(event)
	{
		...
		function watchMouse(event)
		{
			var offsetHolder = touchEvent ? event.originalEvent.touches[0] : event,
				position = availableSpace-Math.max(0, Math.min(availableSpace, initialPosition+(offsetHolder.pageX-mouseX)));
			...
			if (reversed)
			{
				button.css('left', (availableSpace-position-buttonOverflow)+'px');
				offEl.css('left', (switchWidth-position-valuesOverflow)+'px');
				offSpan.css('margin-right', -(availableSpace-position+marginIE7)+'px');
				onEl.css('right', (buttonWidth+position-valuesOverflow)+'px');
			}
			else
			{
				button.css('right', (position-buttonOverflow)+'px');
				onEl.css('left', (switchWidth-position-valuesOverflow)+'px');
				onSpan.css('margin-right', -(availableSpace-position+marginIE7)+'px');
				offEl.css('right', (buttonWidth+position-valuesOverflow)+'px');
			}
