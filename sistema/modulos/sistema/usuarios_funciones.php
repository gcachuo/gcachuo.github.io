<?php
	...
	function usuarios_menuInicio()
	{
		...
		$selectUsuarios = '	SELECT
							...
							FROM
								_perfiles perf
							INNER JOIN _usuarios usuar ON perf.id_perfil = usuar.id_perfil
							WHERE
	...
	function usuarios_formularioUsuario($tipo)
	{
		switch($tipo)
		{
			case "nuevo":
			...
			case "editar":
				...
				$opcionBtn = 'GuardarEdit';
				liberar_bd();
				$selectUsuarioId = "CALL sp_sistema_select_usuario_id(".$_POST["idUsuario"].");";
				$usuarioId = consulta($selectUsuarioId);				
				$user = siguiente_registro($usuarioId);
				$pagina = '<input type="hidden" name="idUsuario" value="'.$_POST["idUsuario"].'" />';
				$boton = 'Editar Usuario';	
				$alertPass = '	<div class="alert alert-dismissable alert-danger">
									<strong>Dejar ambos campos en blanco para no modificar</strong>
									<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
								</div>';
			break;
		}
		
		//LISTA DE PERFILES DEL AGENTE
		liberar_bd();
		$selectPerfil = 'CALL sp_sistema_lista_perfiles_agente('.$_SESSION['idAgenteActual'].');';
	...
	function usuarios_guardaUsuario()
	{
		if($_POST["tipo"] == "nuevo")
		{
			...
			if($ctaloginUsuario == 0)
			{
				...
				if($insertUsuario)
				{
					//CHECAMOS SI EL PERFIL ES SUB AGENTE
					liberar_bd();
					$selectPerfilSubAgente = 'CALL sp_sistema_select_datos_perfil('.$_POST["id_perfil"].');';
					$perfilSubAgente = consulta($selectPerfilSubAgente);
					$perfSubAgen = siguiente_registro($perfilSubAgente);
					if($perfSubAgen["tipo"] == 3)
					{
						//ULTIMO USUARIO INSERTADO
						liberar_bd();
						$ultimoUser = 'CALL sp_sistema_select_ultimo_usuario('.$_SESSION[$varIdUser].');';
						$ultUser = consulta($ultimoUser);
						$idUser = siguiente_registro($ultUser);						
						
						//INSERTAMOS EL SUB AGENTE
						liberar_bd();
						$insertSubAgente = 'CALL sp_sistema_insert_subAgente('.$idUser["id"].', '.$_SESSION[$varIdUser].');';
						$insertSubAgen = consulta($insertSubAgente);
					}
					
					/*$error='Se ha creado el usuario exitosamente.';
					$msj = sistema_mensaje("exito",$error);*/
					$res= $msj.usuarios_menuInicio();					
				}
				else
				{
					$error='No se ha podido crear el usuario.';
					$msj = sistema_mensaje("error",$error);
					$res= $msj.usuarios_formularioUsuario($_POST["tipo"]);					
				}
	...
	function usuarios_eliminaUsuario()
	{
		liberar_bd();
