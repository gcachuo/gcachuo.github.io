...
class image {
	...
	public function resize($width = NULL,$height = NULL) 
	{
		if(isset($this->source["tmp_name"]) && file_exists($this->source["tmp_name"])) 
		{
			...
			{
				case "image/jpeg" : 
					$created = imagecreatefromjpeg($this->source["tmp_name"]);					
				break;
				case "image/gif"  : 
					$created = imagecreatefromgif($this->source["tmp_name"]); 
					imagecolortransparent($new, imagecolorallocatealpha($new, 0, 0, 0, 127));
    				imagealphablending($new, false);
    				imagesavealpha($new, true);
				break;
				case "image/png"  : 
					$created = imagecreatefrompng($this->source["tmp_name"]); 
					imagealphablending($created, true);
					imagesavealpha($created, true);	
				break;
			}
