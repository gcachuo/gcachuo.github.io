 ...
 */
        ...
        function fnCreateButton(iFilterLength, iMaxLenght) {
        var sCSSClass = "button_filter btn-default";
                label = label.replace(/(^\s*)|(\s*$)/g, "");
                var currentFilter = oTable.fnSettings().aoPreSearchCols[i].sSearch;
                var search_init = 'search_init ';
                var inputvalue = label;
                if (currentFilter != '' && currentFilter != '^') {
        if (currentFilter.charAt(0) == '^')
                inputvalue = currentFilter.substr(1); //ignore trailing ^
                else
                inputvalue = currentFilter;
                search_init = '';
        ...
        var input = $('<input type="button" class="' + search_init + sCSSClass + '" value="' + inputvalue + '" rel="' + i + '"/>');
                if (iMaxLenght != undefined && iMaxLenght != - 1) {
        input.attr('maxlength', iMaxLenght);
        }
        th.html(input);
        ...
        /* Filter on the column (the index) of this element */
        oTable.fnFilter("", _fnColumnIndex(index), regex, smart); //Issue 37
                fnOnFiltered();
        });
        }
        ...
        //$(this).removeClass("search_init");
                ...
                $(this).addClass("search_init");
                        this.value = asInitVals[index];
                         oTable.fnFilter("", _fnColumnIndex(index)); //Issue 37
                fnOnFiltered();
        ...
        function fnCreateDateRangeInput2(oTable) {

        var aoFragments = sRangeFormat.split(/[}{]/);
                th.html("");
                //th.html(_fnRangeLabelPart(0));
                var sFromId = oTable.attr("id") + '_range_from_' + i;
                var from = $('<input type="text" class="date_range_filter" id="' + sFromId + '" rel="' + i + '" value="' + addDate(getToday(), "day", - 15) + '"/>');
                from.datepicker();
                //th.append(from);
                //th.append(_fnRangeLabelPart(1));
                var sToId = oTable.attr("id") + '_range_to_' + i;
                var to = $('<input type="text" class="date_range_filter" id="' + sToId + '" rel="' + i + '" value="' + addDate(getToday(), "day", 15) + '"/>');
                //th.append(to);
                //th.append(_fnRangeLabelPart(2));
        ...
        th.wrapInner('<span class="filter_column filter_date_range" />');
                ...
                oTable.dataTableExt.afnFiltering.push(
                        ...
                        if (oTable.attr("id") != oSettings.sTableId)
                                ...
                                var today = getToday();
                                var dEndDate = to.datepicker("getDate");
                                var todayplus = addDate(getToday(), "day", 15);
                                if (dStartDate == null && dEndDate == null) {
                        return true;
                        }
        ...
        oTable.fnDraw();
                fnOnFiltered();
                document.getElementById('tablaRecordar_range_from_4').value = "";
                document.getElementById('tablaRecordar_range_to_4').value = "";
        })
        }
        ...
        if (aData == null)
                ...
                var r = '<select style="height: 5%" class="search_init select_filter form-control" rel="' + i + '"><option value="" class="search_init">' + sLabel + '</option>';
                if (bMultiselect) {
        r = '<select style="height: 5%" class="search_init select_filter form-control" rel="' + i + '" multiple>';
        }
        var j = 0;
        ...
        var oTable = this;
                ...
                sPlaceHolder: "foot",
                        ...
                        iFilteringDelay: 500,
                        aoColumns: null,
                        sRangeFormat: "DE {from} <br/><br/>&nbsp; A {to}",
                        sDateFromToken: "from",
                        sDateToToken: "to"
                };
                ...
                case "null":
                        ...
                        break;
                        case "date-range2":
                        fnCreateDateRangeInput2(oTable);
                        break;
                        case "checkbox":
                        ...
                        fnCreateButton(aoColumn.iFilterLength, aoColumn.iMaxLenght);
                        break;
                        case "text":
                        default:
                        bRegex = (aoColumn.bRegex == null ? false : aoColumn.bRegex);
